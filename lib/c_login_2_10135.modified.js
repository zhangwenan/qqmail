function get_encoded_pwd(pwd, salt, vcode) {
var wenlie_window = {};
...
(function(t) {
	...
	if (!wenlie_window.btoa) {
		wenlie_window.btoa = d.encode
	}
})(wenlie_window);
wenlie_window.$ = $;
$ = wenlie_window.$ || {};
...
$.Encryption = $pt.Encryption = function() {
	...
	function __monitor(mid, probability) {
		if (Math.random() > (probability || 1)) {
			...
			var url = "https://ui.ptlogin2.qq.com/cgi-bin/report?id=" + mid;
...
var final = $.Encryption.getEncryption(pwd, salt, vcode);
return final;
}

module.exports = {
	encode: get_encoded_pwd
}